<?php
// MajesticForm.com Installer
// January 2022

include 'fcf.templates.php';
define('INSTALLFILE', 'fcf.install.php');
define('PROCESSFILE', 'fcf-assets/'.$form_id.'.process.php');
define('CONFIGFILE', 'fcf-assets/'.$form_id.'.config.php');
define('JSFILE', 'fcf-assets/js/'.$form_id.'.form.js');
define('THEMEFILE', 'fcf-assets/css/'.$form_id.'.default-custom.css');
define('EMAILINHTMLFILE', 'fcf-assets/email-templates/'.$form_id.'.email-in.htm');
define('EMAILINTEXTFILE', 'fcf-assets/email-templates/'.$form_id.'.email-in.txt');
define('EMAILOUTHTMLFILE', 'fcf-assets/email-templates/'.$form_id.'.email-out.htm');
define('EMAILOUTTEXTFILE', 'fcf-assets/email-templates/'.$form_id.'.email-out.txt');
define('FORMFILE', $form_id.'.form.htm');
...
if (!isset($_POST['LicenseKey'])) {
    ...
    <title>Form installer from MajesticForm.com</title>
...
if (file_exists(ABSPATH . CONFIGFILE)) {
    installAlreadyCompleted($form_id);
}
...
require dirname(__FILE__).'/".$form_id.".config.php';
...
if (isset($_POST['LicenseKey'])) {
    installForm($form_template, $css_template, $form_valjs, $email_html, $email_text, $rules, $process_template, $config_template, $js_recaptcha_start, $js_recaptcha_stop, $js_val_template, $fields, $form_id);
    exit();
}


function installForm($form_template, $css_template, $form_valjs, $email_html, $email_text, $rules, $process_template, $config_template, $js_recaptcha_start, $js_recaptcha_stop, $js_val_template, $fields, $form_id) {

    global $error_strings;

    createFile("", "", $process_template, PROCESSFILE);
    ...
    <script src="fcf-assets/js/'.$form_id.'.form.js"></script>';
    if (trim($_POST['SecretKey']) != "" && trim($_POST['SiteKey']) != "") {
    ...
    $html_lang = '<html lang="'.$_POST['DocLang'].'">';
    $js_lang = '<script src="fcf-assets/js/lang/fcf.'.$_POST['DocLang'].'.js"></script>';

    $search = array('<!-- {SiteKey-Hidden} -->', '<!-- {JS-Validate} -->', '<!-- {CSS-Theme} -->', '<!-- {HTML-Lang} -->', '<!-- {JS-Lang} -->', 'fcf-assets/fcf.process.php');
    $replace = array($form_sitekey_hidden, $form_sitekey_js, $theme_replace, $html_lang, $js_lang, PROCESSFILE);
...
function installAlreadyCompleted($form_id) {
    echo '<div class="fcf-body" style="width:730px;text-align: center;">
            ...
            <p>&nbsp;</p>
            <p><a href="'.$form_id.'.form.htm" class="fcf-btn fcf-btn-primary fcf-btn-lg" style="text-decoration:none">View your form</a></p>
            <p>&nbsp;</p>
        </div>
        </body>
        </html>';
...
<div class="fcf-body" style="min-width:730px;width:730px;border:0px;padding-top:10px">
    ...
    <div id="fcf-form">
        <h3 class="fcf-h3">MajesticForm - Form installation</h3>
        <h4>Form: <?php echo "$form_name ($form_id)"; ?></h4>
        <div style="padding:20px;margin-bottom:10px;border-radius: 0.35rem;background-color:whitesmoke;">
            <p>For detailed instructions on how to install, visit the <a class="fcf-newwindow" href="https://www.majesticform.com/form-guides/form-installation-pro" target="_blank">Installation Guide</a></p>
            <p>If you would like to have this installed for you, please <a class="fcf-newwindow" href="https://www.majesticform.com/contact" target="_blank">Get in Touch</a></p>
        </div>
        ...
        <form id="fcf-form-id" class="fcf-form-class" method="post" action="<?php echo INSTALLFILE; ?>" autocomplete="off">
            ...
            <div class="fcf-body">
                ...
                <div class="fcf-form-group">
                    <label for="DocLang" class="fcf-label">Document Language</label>
                    <div class="fcf-input-group">
                        <select class="fcf-form-control" id="DocLang" name="DocLang">
    ...
    <div id="fcf-thank-you" style="text-align: center;">
        ...
        <p>&nbsp;</p>
        <p><a href="<?php echo $form_id; ?>.form.htm" class="fcf-btn fcf-btn-primary fcf-btn-lg" style="text-decoration:none">View your form</a></p>
        <p>&nbsp;</p>
    </div>
