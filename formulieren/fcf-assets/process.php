...
//*********************************************
// BLOCK FOR COPYRIGHT DATE BOTTOM MAIL PART 1
//*********************************************        
    }
    $year = date("Y");
    $time = date(EMAIL_DATEFORMAT,mktime(date("H")+(HOURS_OFFSET),date("i"),date("s"),date("m"),date("d"),date("y")));
    $body = str_replace(array('{IP}','{SCORE}','{YEAR}','{TIME}'), array(getUserIp(),$score,$year,$time), $body);
    return str_replace('{CREDIT}', getSsValue($ss), $body);
}
// END BLOCK COPYRIGHT DATE 1
...
//*********************************************
// BLOCK FOR COPYRIGHT DATE BOTTOM MAIL PART 2
//*********************************************    
  $year = date("Y");
    $time = date(EMAIL_DATEFORMAT,mktime(date("H")+(HOURS_OFFSET),date("i"),date("s"),date("m"),date("d"),date("y")));
    $s = array('{EMAIL_OUT_FROM_NAME}','{YEAR}','{TIME}');
    $r = array(EMAIL_OUT_FROM_NAME,$year,$time);
