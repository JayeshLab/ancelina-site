...
class Ajaxify {
  constructor(options) {
    ...
    // Dispatch document DOMContentLoaded event
    Ay.triggerDCL = () => {
      if (Ay.s.DCLDelay !== false) {
        if (Ay.s.DCLDelay === 0) {
          //fire DOMContentLoaded straight away
          ...
          setTimeout(function () {
            //use setTimeout only for milliseconds > 0
            document.dispatchEvent(
              new Event("DOMContentLoaded", {
                bubbles: true,
                cancelable: true,
    ...
    Ay.trigger = (t, e) => {
      ...
      if (t === "render") Ay.triggerDCL();
      if (t === "loaded") Ay.triggerLoaded();
    };
      ...
      let iFn = function (a, b, c = false) {
        if ((this === document || this === window) && (a == "DOMContentLoaded" || a == "load")) {
          if (c == false) {
            ...
            c = Object.assign(c, { once: true });
          }
        }
        return this.ael(a, b, c);
    ...
    if (Ay.s.intevents) Ay.intevents();
...
class AddAll {
  ...
  a(sl, pk) {
    ...
    if (this.TCS.length) {
      Promise.all(this.TCS)
        .then(() => Ay.trigger("loaded"))
        .catch((e) => console.log(`error in loading script ${e.target.src}`));
    }
  }
  ...
  removeScript() {
    qa(
      (this.PK == "href" ? 'link[href*="!"]' : 'script[src*="!"]') //remove all scripts including relative URLS
        .replace("!", this.u.substring(this.u.lastIndexOf("/") + 1))
    ).forEach((e) => prC(e));
  }
}
...
let myAjaxify = new Ajaxify({
  elements: "#main-content, #navbarCollapse, #footer-stay",
  verbosity: 2,
  prefetchoff: !0,
  scrolltop: !0,
  pluginon: !0,
  ...
  intevents: !0,
  DCLDelay: 60,
  alwayshints: "pgia.js, menukit.js, fcf.just-validate.min.js, swiper-bundle.min.js, cleave.min.js, smooth-scroll.polyfills.min.js, lazysizes.js, functions.js, index.js",
});
